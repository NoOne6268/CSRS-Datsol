...
class AuthService {
  ...
  Future<void> saveContact(String contact, BuildContext context) async {
    try {
      ...
      if (contacts.docs.isNotEmpty) {
        ...
        if (emergencyContactsQuery.docs.isNotEmpty) {
          ...
          await FirebaseFirestore.instance
              ...
              .set({'contacts': FieldValue.arrayUnion([contact]), 'token' : FieldValue.arrayUnion([token])}, SetOptions(merge: true));
        } else {
          // Create a new document for the current user with the new contact
          await FirebaseFirestore.instance
              .collection('emergency_contacts')
              .add({'username': currentUser.email, 'contacts': [contact] , 'token' : [token] });
        }
        ...
        // // Store the ScaffoldMessengerState in a variable
        // final scaffoldMessenger = ScaffoldMessenger.of(context);
        //
        // // Show the SnackBar using the stored ScaffoldMessengerState
        // scaffoldMessenger.showSnackBar(const SnackBar(
        //     content: Text('No user found with the specified email')));
      }
    } catch (e) {
      print("Error saving user info: $e");
    }
